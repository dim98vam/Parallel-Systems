...
#define level 5 //calculated log base 2 of the maximum desired threads active at the same time
...
void calculateDist(int dim, int size,double arrayList[size][dim],double* dist){
     ...
     if(chunk>200000){
      ...
      #pragma omp parallel for schedule(static,chunk) num_threads(numOfThreads)\
      shared(arrayList,dist,dim) private(i)
            for( i=0;i<size-1;++i){
             double distSqrd=0.0;
             int y;
...
//validated
vptree * createVpTreeOpenMP(int dim,int size,int index[size],double list[size][dim],double* dist){
    ...
    #pragma omp parallel num_threads(2)\
     shared(dim,size,index,list,dist,node)
     {
        #pragma omp single nowait
        {
         if(size%2!=0){
           #pragma omp task
           {
             node->right=createVpTreeOpenMP(dim,(size-1)/2,index+(size-1)/2,list+(size-1)/2,dist+(size-1)/2);
           }
           node->left=createVpTreeOpenMP(dim,(size-1)/2,index,list,dist);
         }
         else{
           #pragma omp task
           {
             node->right=createVpTreeOpenMP(dim,(size-1)/2,index+(size-1)/2+1,list+(size-1)/2+1,dist+(size-1)/2+1);
           }
           node->left=createVpTreeOpenMP(dim,(size-1)/2+1,index,list,dist);
          }
         }
        }
     }
...
vptree* buildvp(double* X,int n,int d){
     ...
     //creating vp
     vptree* root=createVpTreeOpenMP(d,n,index,(double**)list,dist);
...
int main(){
    ...
    double p_time = (double)((endwtime.tv_usec - startwtime.tv_usec)/1.0e6
