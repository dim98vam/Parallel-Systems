...
//validated
void calculateDistNonThreaded(int dim, int size,double arrayList[size][dim],double* dist){
...
//Validated
void calculateDistThreaded(int dim, int size,double arrayList[size][dim],double* dist){

  int chunk;
  int holder;
  for(holder=numOfThreads;holder>1;holder/=2){
    chunk=(size-1)/holder;
    if(chunk>200000){

      //variables to be used with threading
      pthread_t callThd[holder-1];
      int i;
      int rc;
      void* status;
      distData* values=(distData*)malloc(holder*sizeof(distData));
      ...
      values[numOfThreads-1].size=((size-1)-(holder-1)*chunk);
      values[numOfThreads-1].array=(arrayList+chunk*(holder-1));
      values[numOfThreads-1].dist1=(dist+chunk*(holder-1));
      values[numOfThreads-1].ref=*(arrayList+size-1);


      calculateDist((void*)&values[holder-1]);
      ...
      return;
        ...
        calculateDistNonThreaded(dim,size,arrayList,dist);
}
...
double findMedian(int dim, int size,double arrayList[size][dim],int index[size],double* dist){
    ...
    calculateDistThreaded(dim,size,arrayList,dist);
...
void *createVpTreeThreaded(void* data){
     ...
     node->VpId=idx[s-1];
     node->coord=list[s-1];
     node-> mu= findMedian(argstruct->dim,s,list,idx,argstruct->dist);
     *(argstruct->position)=node;
     ...
     if(counter<NUM_THREADS){
        ...
        if(s%2!=0){
         args argsL={argstruct->dim,(s-1)/2,idx,list,&(node->left),argstruct->dist};
         argsLeft=argsL;

         args argsR={argstruct->dim,(s-1)/2,idx+(s-1)/2,list+(s-1)/2,&(node->right),(argstruct->dist)+(s-1)/2};
         argsRight=argsR;
        }
        else{
         args argsL={argstruct->dim,(s-1)/2+1,idx,list,&(node->left),argstruct->dist};
         argsLeft=argsL;

         args argsR={argstruct->dim,(s-1)/2,idx+(s-1)/2+1,list+(s-1)/2+1,&(node->right),(argstruct->dist)+(s-1)/2+1};
         argsRight=argsR;
        }
        ...
        if(s%2!=0){
         node->left=createVpTree(argstruct->dim,(s-1)/2,idx,list,argstruct->dist);
         node->right=createVpTree(argstruct->dim,(s-1)/2,idx+(s-1)/2,list+(s-1)/2,(argstruct->dist)+(s-1)/2);
        }
        else{
         node->left=createVpTree(argstruct->dim,(s-1)/2+1,idx,list,argstruct->dist);
         node->right=createVpTree(argstruct->dim,(s-1)/2,idx+(s-1)/2+1,list+(s-1)/2+1,(argstruct->dist)+(s-1)/2+1);
        }
