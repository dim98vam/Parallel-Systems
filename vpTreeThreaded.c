...
void *createVpTreeThreaded(void* data){
     ...
     node-> mu= findMedian(node->dim,s,list,idx,argstruct->dist);
     node->index=idx[s-1];
     *(argstruct->position)=node;
     ...
     if(counter<NUM_THREADS){
        ...
        if(s%2!=0){
         args argsL={node->dim,(s-1)/2,idx,list,&(node->left),argstruct->dist};
         argsLeft=argsL;

         args argsR={node->dim,(s-1)/2,idx+(s-1)/2,list+(s-1)/2,&(node->right),(argstruct->dist)+(s-1)/2};
         argsRight=argsR;
        }
        else{
         args argsL={node->dim,(s-1)/2+1,idx,list,&(node->left),argstruct->dist};
         argsLeft=argsL;

         args argsR={node->dim,(s-1)/2,idx+(s-1)/2+1,list+(s-1)/2+1,&(node->right),(argstruct->dist)+(s-1)/2+1};
         argsRight=argsR;
        }
        ...
        if(s%2!=0){
         node->left=createVpTree(node->dim,(s-1)/2,idx,list,argstruct->dist);
         node->right=createVpTree(node->dim,(s-1)/2,idx+(s-1)/2,list+(s-1)/2,(argstruct->dist)+(s-1)/2);
        }
        else{
         node->left=createVpTree(node->dim,(s-1)/2+1,idx,list,argstruct->dist);
         node->right=createVpTree(node->dim,(s-1)/2,idx+(s-1)/2+1,list+(s-1)/2+1,(argstruct->dist)+(s-1)/2+1);
        }
...
int main(void)
{
    ...
    int counter=0;
    vptree* root=buildvp(*holder,12,3);
