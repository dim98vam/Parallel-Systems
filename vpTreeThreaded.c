...
void *createVpTreeThreaded(void* data){
     ...
     *(argstruct->position)=node;
     ...
     if(counter<NUM_THREADS){
        ...
        if(s%2!=0){
         node->left=createVpTree(node->dim,(s-1)/2,idx,(argstruct->list));
         node->right=createVpTree(node->dim,(s-1)/2,idx+(s-1)/2,(argstruct->list)+(s-1)/2);
        }
        else{
         node->left=createVpTree(node->dim,(s-1)/2+1,idx,(argstruct->list));
         node->right=createVpTree(node->dim,(s-1)/2,idx+(s-1)/2+1,(argstruct->list)+(s-1)/2+1);
        }
